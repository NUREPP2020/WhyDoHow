...
<main class="main-block1">
    <div>
        ...
        $sql = "UPDATE `stream` SET `view_count` = `view_count` + 1 WHERE `id_stream` = '$postno'";
        $result = mysqli_query($link, $sql);
        $rows = mysqli_fetch_array($result);
        $sql = "SELECT * FROM `user` WHERE `id_user` = '{$rows['id_user']}'";
        $result = mysqli_query($link, $sql);
        $hozainposta = mysqli_fetch_assoc($result);
        $sql = "SELECT * FROM stream WHERE id_stream = $postno";
        if($result = mysqli_query($link, $sql)){
            if(mysqli_num_rows($result) > 0){
                while($rows = mysqli_fetch_array($result))
                {
                    $sql1 = "SELECT * FROM `user` WHERE `id_user` = '{$rows['id_user']}'";
                    $result1 = mysqli_query($link, $sql1);
                    $hozainposta = mysqli_fetch_assoc($result1);
...
<div class="down-info">
    <div class="down-cat"><p>Категории: </p><p>'.$rows['id_category'].'</p></div>
